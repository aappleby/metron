...
class singlecycle_datapath {
 public:
  ...
  void tocktick_regs(logic<1> reset, logic<32> inst, logic<1> regfile_write_enable, logic<32> data_mem_read_data, logic<3> reg_writeback_select, logic<32> alu_result2, logic<2> next_pc_select, logic<1> pc_write_enable) {
    ...
    logic<32> out2 = mux_next_pc_select.out(
      next_pc_select,
      adder_pc_plus_4.result(b32(0x00000004), program_counter.value),
