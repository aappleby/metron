...
class Pinwheel {
 public:
  ...
  signals_p20 delta20(logic<32> p2_data_read, logic<32> p2_reg_alu_result) {
    ...
    signals_p20 sig_p20;
    switch(reg_p2.op) {
      case OP_ALU:     sig_p20.reg_wren = true;  sig_p20.reg_data = p2_reg_alu_result;    break;
      case OP_ALUI:    sig_p20.reg_wren = true;  sig_p20.reg_data = p2_reg_alu_result;    break;
    ...
    reg_p0.pc = reg_p2.pc_sel ? b32(p2_reg_alu_result) : b32(reg_p2.pc + 4);
  ...
  void tick(logic<1> reset_in) {
    ...
    logic<32> prog_read = prog_mem[b14(sig_p20.mem_addr, 2)];

    if (phase == 0) {
      delta01(reg_p0, prog_read, sig_p01, reg_p1);
      next_phase = 1;
    }


    if (phase == 1) {
      logic<32> regs_read_data1 = regs[sig_p01.addr1];
      logic<32> regs_read_data2 = regs[sig_p01.addr2];
      delta12(reg_p1, regs_read_data1, regs_read_data2, sig_p12, reg_p2);
      next_phase = 2;
    }

    if (sig_p12.byte_wren) {
      logic<32> old_mem = data_mem[b15(sig_p12.addr, 2)];
      if (sig_p12.byte_wren[0]) old_mem = (old_mem & 0xFFFFFF00) | (sig_p12.data & 0x000000FF);
      if (sig_p12.byte_wren[1]) old_mem = (old_mem & 0xFFFF00FF) | (sig_p12.data & 0x0000FF00);
      if (sig_p12.byte_wren[2]) old_mem = (old_mem & 0xFF00FFFF) | (sig_p12.data & 0x00FF0000);
      if (sig_p12.byte_wren[3]) old_mem = (old_mem & 0x00FFFFFF) | (sig_p12.data & 0xFF000000);
      data_mem[b15(sig_p12.addr, 2)] = old_mem;
    }
    logic<32> data_read = data_mem[b15(sig_p12.addr, 2)];

    if (phase == 2) {
      sig_p20 = delta20(data_read, reg_p2.alu_result);
      next_phase = 0;
    }

    if (sig_p20.reg_addr && sig_p20.reg_wren) {
      regs[sig_p20.reg_addr] = sig_p20.reg_data;
    }
