...
void CNodeUnion::init() {
  node_union = child("union")->req<CNodeKeyword>();
...
CNodeField* CNodeUnion::get_field(std::string_view name) {
  for (auto f : all_fields) if (f->name == name) return f;
  return nullptr;
}

CNode* CNodeUnion::resolve(std::vector<CNode*> path) {
  auto front = path[0];
